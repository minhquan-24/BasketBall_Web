<h1>Modify Product</h1>

<form action="index.php?area=admin&controller=product&action=update" method="POST">
    <input type="hidden" name="id" value="<?php echo $this->productModel->id; ?>">

    <div class="row">
        <div class="col-md-8">
            <div class="mb-3">
                <label for="name" class="form-label">Tên sản phẩm</label>
                <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($this->productModel->name); ?>" required>
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Mô tả</label>
                <textarea class="form-control" id="description" name="description" rows="5"><?php echo htmlspecialchars($this->productModel->description); ?></textarea>
            </div>
            <div class="mb-3">
                <label for="image_url" class="form-label">URL Hình ảnh chính</label>
                <input type="text" class="form-control" id="image_url" name="image_url" value="<?php echo htmlspecialchars($this->productModel->image_url); ?>">
            </div>
        </div>
        <div class="col-md-4">
            <div class="mb-3">
                <label for="price" class="form-label">Giá (VNĐ)</label>
                <input type="number" class="form-control" id="price" name="price" value="<?php echo htmlspecialchars($this->productModel->price); ?>" required>
            </div>
            <div class="mb-3">
                <label for="category_id" class="form-label">Danh mục</label>
                <select class="form-select" id="category_id" name="category_id" required>
                    <?php foreach ($all_categories as $category): ?>
                        <?php $selected = ($category['id'] == $this->productModel->category_id) ? 'selected' : ''; ?>
                        <option value="<?php echo $category['id']; ?>" <?php echo $selected; ?>>
                            <?php echo htmlspecialchars($category['name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                <label for="manufacturer" class="form-label">Nhà sản xuất</label>
                <input type="text" class="form-control" id="manufacturer" name="manufacturer" value="<?php echo htmlspecialchars($this->productModel->manufacturer); ?>">
            </div>
            <div class="mb-3">
                <label for="usage_type" class="form-label">Loại sân</label>
                <select class="form-select" id="usage_type" name="usage_type">
                    <?php $usage_types = ['Both' => 'Cả hai', 'Indoor' => 'Trong nhà', 'Outdoor' => 'Ngoài trời']; ?>
                    <?php foreach ($usage_types as $key => $value): ?>
                        <?php $selected = ($key == $this->productModel->usage_type) ? 'selected' : ''; ?>
                        <option value="<?php echo $key; ?>" <?php echo $selected; ?>><?php echo $value; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        <hr>

        <h4>Quản lý Size và Số lượng</h4>
        <div id="variants-container">
            <?php if (!empty($product_variants)): ?>
                <?php foreach ($product_variants as $variant): ?>
                    <div class="row align-items-center mb-2 variant-row">
                        <input type="hidden" name="variants[id][]" value="<?php echo $variant['id']; ?>">
                        <div class="col-md-4">
                            <label class="form-label">Size</label>
                            <input type="text" class="form-control" name="variants[size][]" value="<?php echo htmlspecialchars($variant['size']); ?>">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Số lượng</label>
                            <input type="number" class="form-control" name="variants[quantity][]" min="0" value="<?php echo $variant['quantity']; ?>">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label d-block">&nbsp;</label>
                            <button type="button" class="btn btn-danger btn-sm remove-variant-btn">Xóa</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else:  ?>
            <div class="row align-items-center mb-2 variant-row">
                <input type="hidden" name="variants[id][]" value=""> 
                <div class="col-md-4">
                    <label class="form-label">Size</label>
                    <input type="text" class="form-control" name="variants[size][]">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Số lượng</label>
                    <input type="number" class="form-control" name="variants[quantity][]" min="0" value="0">
                </div>
                <div class="col-md-2">
                    <label class="form-label d-block">&nbsp;</label>
                    <button type="button" class="btn btn-danger btn-sm remove-variant-btn">Xóa</button>
                </div>
            </div>
            <?php endif; ?>
        </div>
        <button type="button" id="add-variant-btn" class="btn btn-secondary btn-sm mt-2"><i class="bi bi-plus-circle"></i> Thêm Size khác</button>
        <hr>
    </div>
    <button type="submit" class="btn btn-success">Update</button>
    <a href="index.php?area=admin&controller=product&action=manage" class="btn btn-secondary">Cancel</a>
</form>
<script>
document.addEventListener('DOMContentLoaded', function() {
    const container = document.getElementById('variants-container');
    const addButton = document.getElementById('add-variant-btn');

    const emptyRowHTML = `
        <div class="row align-items-center mb-2 variant-row">
            <input type="hidden" name="variants[id][]" value="">
            <div class="col-md-4">
                <label class="form-label">Size</label>
                <input type="text" class="form-control" name="variants[size][]">
            </div>
            <div class="col-md-4">
                <label class="form-label">Số lượng</label>
                <input type="number" class="form-control" name="variants[quantity][]" min="0" value="0">
            </div>
            <div class="col-md-2">
                <label class="form-label d-block">&nbsp;</label>
                <button type="button" class="btn btn-danger btn-sm remove-variant-btn">Xóa</button>
            </div>
        </div>`;

    function addVariantRow() {
        container.insertAdjacentHTML('beforeend', emptyRowHTML);
    }

    function removeVariantRow(event) {
        if (event.target.classList.contains('remove-variant-btn')) {
             if (container.querySelectorAll('.variant-row').length > 1) {
                event.target.closest('.variant-row').remove();
            } else {
                const lastRow = container.querySelector('.variant-row');
                lastRow.querySelector('input[name="variants[id][]"]').value = '';
                lastRow.querySelector('input[name="variants[size][]"]').value = '';
                lastRow.querySelector('input[name="variants[quantity][]"]').value = '0';
            }
        }
    }
    
    addButton.addEventListener('click', addVariantRow);
    container.addEventListener('click', removeVariantRow);
});
</script>